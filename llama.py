...
class LayerNorm(torch.nn.Module):
    ...
    def _norm(self, x):
        ...
        """
        return (x - x.mean(dim=-1, keepdim=True)) / ((x.var(dim=-1, keepdim=True, unbiased = False) + self.eps) ** 0.5)
...
class Attention(nn.Module):
    ...
    def compute_query_key_value_scores(self,
        ...
        prod = query @ key.transpose(-2, -1) / (self.head_dim ** 0.5)
...
class LlamaLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        ff_norm_x = self.ffn_norm(x)
        x = x + self.feed_forward(ff_norm_x)
        return x
